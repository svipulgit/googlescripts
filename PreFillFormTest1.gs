...
function betterBuildUrls() {
  var dbSheetName = "Backup";
  var emailDataSheetName = "Email Template";
  var statusColumnIndex = "U1";
  var timeStampColumnIndex = "V1";
  var sendEmailTo = "Father"; // or "Mother"
  ...
  for (var i = 0; i < objects.length; ++i) {
    ...
    if (sendEmailTo == "Father") {
      var emailAddr = rowData.fathersEmail;
    } else {
      var emailAddr = rowData.mothersEmail;
    }
    if (!emailAddr) {
      continue;
    }
    ...
    if (rowData.emailStatus.toLowerCase() != "send") {
      continue;
    }
    ...
    var response = formItem.createResponse(rowData.studentId);
    formResponse.withItemResponse(response);

    // Prefill First Name
    var formItem = items[1].asTextItem();
    var response = formItem.createResponse(rowData.studentsFirstName);
    formResponse.withItemResponse(response);

    // Prefill Birthdate
    formItem = items[3].asDateItem();
    response = formItem.createResponse(rowData.birthDate);
    formResponse.withItemResponse(response);
    ...
    var emailSubject = "Hi " + rowData.studentsFirstName + ", This is your registration link";
